        ...
        for rule in rules :
            if 'outdated_replication_dataset' not in rule['name'] :
                if 'Rules' not in RUCIO :
                    ...
                    if '-test' not in rule['scope']:
                        ...
                        RUCIO['Rules']['total_test_rules'] += 1
                        if rule['state'] == 'REPLICATING' : 
                            RUCIO['Rules']['total_test_replicating'] += 1
                        elif rule['state'] == 'STUCK' :
            ...
            if 'AllRules' not in RUCIO : 
                ...
                if '-test' not in rule['scope']:
                    ...
                    RUCIO['AllRules']['total_test_rules'] += 1
                    if rule['state'] == 'REPLICATING' : 
                        RUCIO['AllRules']['total_test_replicating'] += 1
                    elif rule['state'] == 'STUCK' :
                        RUCIO['AllRules']['total_test_stuck'] += 1
                    elif rule['state'] == 'OK' :
                        RUCIO['AllRules']['total_test_ok'] += 1       
                    elif rule['state'] == 'INJECT' :
                        RUCIO['AllRules']['total_test_inject'] += 1   
            ...
            if 'Grouping' not in RUCIO : 
                ...
                if '-test' not in rule['scope']:
                    ...
                    if rule['did_type'] == 'CONTAINER' : 
                        RUCIO['Grouping']['test_container'] += 1
                    elif rule['did_type'] == 'DATASET' :
                        RUCIO['Grouping']['test_dataset'] += 1
                    elif rule['did_type'] == 'FILE' :
                        RUCIO['Grouping']['test_file'] += 1                     
            else :  
                if '-test' not in rule['scope']:
                    if rule['did_type'] == 'CONTAINER' : 
                        RUCIO['Grouping']['container'] += 1
...
def stats_usage_rules(all_rses=rses_lists) :    
    ...
    for x_rse in all_rses :
        rses = list(client.get_local_account_usage(account='bruzzese',rse=x_rse))[0]
        if rses['bytes'] != 0 :
